...
void CharacterController::Update(float deltaTime) {
    ...
    if ((InputEngine::IsKeyDown(GLFW_KEY_W)) && (_canJump == true)) {
        _body->SetLinearVelocity(glm::vec3(_body->GetLinearVelocity().x, _body->GetLinearVelocity().y, _impulse.z));
        _canJump = false;
    }
    if ((!InputEngine::IsKeyDown(GLFW_KEY_A)) && 
        (!InputEngine::IsKeyDown(GLFW_KEY_D)) && 
        (!InputEngine::IsKeyDown(GLFW_KEY_W)) && 
        (_platform != "") && 
        (_platform != "BeatGem")) {
        _body->SetLinearVelocity(glm::vec3(0.0f, 0.0f, 0.0f));
    }
    glm::vec3 currentPosition = glm::vec3(GetGameObject()->GetPosition());

    GetGameObject()->SetPostion(glm::vec3(currentPosition.x, 5.61f, currentPosition.z));
